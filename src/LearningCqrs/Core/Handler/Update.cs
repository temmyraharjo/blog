...
public class Update
{
    ...
    public abstract class
        UpdateDocumentHandler<TInput, TEntity> : IRequestHandler<UpdateDocument<TInput, TEntity>, TEntity>
        where TInput : class, IRequest<TEntity>
            ...
            CancellationToken cancellationToken)
        {
            var entity = await Repository.Context.Set<TEntity>()
                .SingleAsync(x => x.Id == request.Id,
                    cancellationToken);
            var valid = await new RowVersionValidator<TEntity>(request.RowVersion).ValidateAsync(entity,
                cancellationToken);
            if (!valid.IsValid)
            {
